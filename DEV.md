...
4. 接入真实天气 API，具备联网查询实时天气与未来 3 天预报的能力。
...
1. **用户管理**：账号注册、登录、重置密码。
2. **天气展示**：主页展示当前城市实时天气、当日最高最低温、未来 3 天预报。
...
3. 主界面下方展示未来 3 天天气预报，每条包含日期、白天/夜间天气现象、高低温、风向风力。
...
5. 进入 `MainActivity` 后自动加载当前城市实时天气与 3 天预报。
...
2. **未来多日天气预报接口**：返回未来 3 天每日白天/夜间的天气、温度、风向风力。
...
若返回多于 3 天，仅取前 3 天；若少于 3 天，取全部并在 UI 层兼容展示。
...
**页面目标**：作为 APP 主视觉入口，展示当前城市实时天气与未来 3 天预报；同时通过统一风格的侧边栏完成用户概要、设置入口与城市管理。
...
6. **未来 3 天预报卡片**：
   - 标题"未来 3 天"。
   - `RecyclerView` 横向或纵向 3 条，每条显示日期（周几 + 月日）、天气图标、高/低温、风向风力。
...
- 未来 3 天预报详细卡片：每天一行，展示日期、白天/夜间天气、高/低温、白天/夜间风向、白天/夜间风力。
...
3. 需要重点验证的页面元素包括：主页顶部温度区、未来 3 天预报卡片、搜索结果列表、设置页账号操作区、详情页数据卡片、登录注册输入区。
